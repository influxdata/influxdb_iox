...
pub trait Database: Debug + Send + Sync {
    ...
    /// Returns the a plan that lists the names of tables in this
    ...
    /// Returns the names of "tag" columns (as defined in the data
    /// written via `write_lines`)) column names in this database which
    /// store tags (as defined in the ParsedLines when written), and
    /// pass the conditions specified by `predicate`.
    ...
    /// optional predicates and pass the conditions specified by
    /// `predicate`.
    ...
    /// conditions specified by `predicate` and which form logical
    /// time series
    ///
    /// A time series is defined by the unique values in a set of
    /// "tag_columns" for each field in the "field_columns"
