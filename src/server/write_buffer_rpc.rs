...
use crate::server::rpc::{expr::convert_predicate, input::GrpcInputs};
...
/// shutdown.
///
/// Database storage is handled by `storage` and all plans are
/// executed using `executor`
    ...
    T: DatabaseStore + 'static,
{
    tonic::transport::Server::builder()
        ...
        .add_service(StorageServer::new(GrpcService::new(storage, executor)))
        .serve(bind_addr)
        .await
        .context(ServerError {})
}
...
mod tests {
        ...
        let test_cases = vec![
            ...
            TestCase {
                ...
                expected_request: ColumnNamesRequest {
                    table: None,
                    range: None,
                    predicate: Some("foo0".into()), // TODO fill this in with the appropriate translation
                },
            },
            // ---
            // Timestamp + Predicate
            ...
            TestCase {
                ...
                expected_request: ColumnNamesRequest {
                    table: None,
                    range: Some(StorageTimestampRange::new(150, 200)),
                    predicate: Some("foo1".into()), // TODO fill this in with the appropriate translation
                },
            },
        ];
        ...
        let test_cases = vec![
            ...
            TestCase {
                ...
                expected_request: ColumnNamesRequest {
                    table: Some("m3".into()),
                    range: None,
                    predicate: Some("foo2".into()), // TODO fill this in with the appropriate translation
                },
            },
            // ---
            // Timestamp + Predicate
            ...
            TestCase {
                ...
                expected_request: ColumnNamesRequest {
                    table: Some("m4".into()),
                    range: Some(StorageTimestampRange::new(150, 200)),
                    predicate: Some("foo3".into()), // TODO fill this in with the appropriate translation
                },
            },
        ];
    ...
    struct Fixture {
        ...
        test_executor: Arc<StorageExecutor>,
    }
