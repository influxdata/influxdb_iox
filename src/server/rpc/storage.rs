    ...
    T: DatabaseStore + 'static,
{
    ...
    async fn tag_values(
        ...
        // Special case a request for 'tag_key=_measurement"
        let response = if tag_key.is_measurement() {
            ...
            if predicate.is_some() {
                unimplemented!("tag_value special case ,easurement names with general predicate");
            }
    ...
    T: DatabaseStore,
{
    ...
    println!("AAL running read_filter with predicate {:?}", predicate);
...
async fn convert_series_set(
    ...
    while let Some(series_set) = rx.recv().await {
        ...
        if let Err(e) = &response {
            error!("Error converting series set: {:?}", e)
        }
