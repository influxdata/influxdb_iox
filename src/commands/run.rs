...
pub struct Config {
    ...
    #[structopt(long = "--log_format", env = "LOG_FORMAT", default_value = "full", verbatim_doc_comment)]
    pub log_format: tracing::LogFormat,

    /// OpenTelemetry trace exporter.
    ///
    /// Can be one of: jaeger, otlp
    ///
    /// Disabled by default.
    /// When enabled, additional flags are required.
    #[structopt(long = "--traces-exporter", env = "TRACES_EXPORTER")]
    pub traces_exporter: Option<tracing::TracesExporter>,
    ...
    /// Endpoint as host:port for the OTLP collector.
    ///
    /// Protocol is gRPC. HTTP is not supported.
    ///
    /// Only used if TRACES_EXPORTER is "otlp".
    ///
    /// https://github.com/open-telemetry/opentelemetry-specification/blob/v1.1.0/specification/sdk-environment-variables.md#otlp-exporter
    #[structopt(
    ...
    /// OpenTelemetry traces Jaeger agent network hostname.
    ///
    /// Protocol is Thrift/Compact over UDP.
    ///
    /// Only used if TRACES_EXPORTER is "jaeger".
    ///
    /// https://github.com/open-telemetry/opentelemetry-specification/blob/v1.1.0/specification/sdk-environment-variables.md#jaeger-exporter
    #[structopt(
    ...
    /// OpenTelemetry traces Jaeger agent network port.
    ///
    /// Protocol is Thrift/Compact over UDP.
    ///
    /// Only used if TRACES_EXPORTER is "jaeger".
