...
use internal_types::schema::{InfluxColumnType, InfluxFieldType, TIME_COLUMN_NAME, IOxValueType};
...
pub enum Error {
    ...
    PBColumnSemanticTypeInvalid { column_name: String, semantic_type: i32 },
...
pub fn pb_to_entry(database_batch: &pb::DatabaseBatch) -> Result<Entry> {
    ...
    let partition_write = entry_fb::PartitionWrite::create(
        &mut fbb,
        &entry_fb::PartitionWriteArgs { key: Some(partition_key), table_batches: Some(table_batches) },
    );
    ...
    let entry = entry_fb::Entry::create(&mut fbb, &entry_fb::EntryArgs {
        operation_type: entry_fb::Operation::write,
...
fn pb_check_table_batch_column_types(table_batch: &pb::TableBatch) -> Result<()> {
    ...
    for column in &table_batch.columns {
        match pb::column::SemanticType::from_i32(column.semantic_type).unwrap() {
            pb::column::SemanticType::Iox => { iox_column_detected = true; }
            pb::column::SemanticType::Field => { lp_field_detected = true; }
            pb::column::SemanticType::Tag => { lp_tag_detected = true; }
    ...
    match (iox_column_detected, lp_tag_detected, lp_field_detected, time_nontime_detected, time_time_detected) {
        (true, false, false, _, _) => Ok(()), // Expected IOx column set
        (false, _, true, false, true) => Ok(()), // Expected line protocol column set
        (true, true, _, _, _) =>
            Err(Error::PBSemanticTypeConflict { message: "IOx column incompatible with line protocol tag column".to_string() }),
        (true, _, true, _, _) =>
            Err(Error::PBSemanticTypeConflict { message: "IOx column incompatible with line protocol field column".to_string() }),
        (_, _, true, true, _) =>
            Err(Error::PBSemanticTypeConflict { message: "line protocol field column incompatible with time column not named 'time'".to_string() }),
        (_, _, true, _, false) =>
            Err(Error::PBSemanticTypeConflict { message: "line protocol field column requires time column named 'time'".to_string() }),
        (_, true, false, _, _) =>
            Err(Error::PBSemanticTypeConflict { message: "line protocol tag column requires at least one line protocol field column".to_string() }),
        _ => unreachable!(),
    }
}

fn pb_table_batch_to_fb<'a>(fbb: &mut FlatBufferBuilder<'a>, table_batch: &pb::TableBatch) -> Result<flatbuffers::WIPOffset<entry_fb::TableWriteBatch<'a>>> {
    pb_check_table_batch_column_types(table_batch)?;
    let mut columns = Vec::with_capacity(table_batch.columns.len());
    for column in &table_batch.columns {
        columns.push(pb_column_to_fb(fbb, &column, table_batch.row_count as usize)?);
    }

    let columns = fbb.create_vector(&columns);
    let name = fbb.create_string(&table_batch.table_name);
    Ok(entry_fb::TableWriteBatch::create(fbb,
                                         &entry_fb::TableWriteBatchArgs {
                                             name: Some(name),
                                             columns: Some(columns),
                                         }))
...
fn pb_column_to_fb<'a>(fbb: &mut FlatBufferBuilder<'a>, column: &pb::Column, table_length: usize) -> Result<flatbuffers::WIPOffset<entry_fb::Column<'a>>> {
    let null_mask_cardinality: usize = column.null_mask.iter().map(|b| b.count_ones() as usize).sum();
    ...
    let values = column.values.as_ref().ok_or(Error::PBColumnContainsOnlyNullValues { column_name: column.column_name.clone() })?;
    let (logical_column_type, values_type, values) = if values.i64_values.len() > 0 {
        let logical_column_type = match pb::column::SemanticType::from_i32(column.semantic_type) {
            ...
            _ => { return Err(Error::PBColumnSemanticTypeInvalid { column_name: column.column_name.clone(), semantic_type: column.semantic_type as i32 }); }
        };
        ...
        let values = entry_fb::I64Values::create(fbb, &entry_fb::I64ValuesArgs { values: Some(values) });

        (logical_column_type, entry_fb::ColumnValues::I64Values, values.as_union_value())
        ...
        let values = entry_fb::F64Values::create(fbb, &entry_fb::F64ValuesArgs { values: Some(values) });

        (logical_column_type, entry_fb::ColumnValues::F64Values, values.as_union_value())
        ...
        let values = entry_fb::U64Values::create(fbb, &entry_fb::U64ValuesArgs { values: Some(values) });

        (logical_column_type, entry_fb::ColumnValues::U64Values, values.as_union_value())
        ...
        let values = {
            ...
            for value in &values.string_values {
                let fb_value = if value.len() < 1000 {
                    *fb_reference_by_value.entry(value).or_insert_with_key(|v| fbb.create_string(v))
                } else {
        ...
        let values = entry_fb::StringValues::create(fbb, &entry_fb::StringValuesArgs { values: Some(values) });

        (logical_column_type, entry_fb::ColumnValues::StringValues, values.as_union_value())
        ...
        let values = entry_fb::BoolValues::create(fbb, &entry_fb::BoolValuesArgs { values: Some(values) });

        (logical_column_type, entry_fb::ColumnValues::BoolValues, values.as_union_value())
            ...
            for value in &values.bytes_values {
                let fb_value = if value.len() < 1000 {
                    *fb_reference_by_value.entry(value).or_insert_with_key(|v| {
                        ...
                        entry_fb::BytesValue::create(fbb, &entry_fb::BytesValueArgs { data: Some(v) })
                    })
                } else {
                    let value = fbb.create_vector(value);
                    entry_fb::BytesValue::create(fbb, &entry_fb::BytesValueArgs { data: Some(value) })
                };
                fb_values.push(fb_value);
            }
            if missing_values > 0 {
        ...
        let values = entry_fb::BytesValues::create(fbb, &entry_fb::BytesValuesArgs { values: Some(values) });

        (logical_column_type, entry_fb::ColumnValues::BytesValues, values.as_union_value())
    } else {
        return Err(Error::PBColumnContainsOnlyNullValues { column_name: column.column_name.clone() });
    };
    ...
    pub fn influx_type(&self) -> InfluxColumnType {
        match self.fb.values_type() {
            entry_fb::ColumnValues::I64Values => match self.fb.logical_column_type() {
                ...
                entry_fb::LogicalColumnType::Field => InfluxColumnType::Field(InfluxFieldType::Integer),
            ...
            entry_fb::ColumnValues::F64Values => match self.fb.logical_column_type() {
                ...
                entry_fb::LogicalColumnType::Field => InfluxColumnType::Field(InfluxFieldType::Float),
                _ => unreachable!(),
            },
            entry_fb::ColumnValues::U64Values => match self.fb.logical_column_type() {
                ...
                entry_fb::LogicalColumnType::Field => InfluxColumnType::Field(InfluxFieldType::UInteger),
                _ => unreachable!(),
            },
            entry_fb::ColumnValues::StringValues => match self.fb.logical_column_type() {
                ...
                entry_fb::LogicalColumnType::Field => InfluxColumnType::Field(InfluxFieldType::String),
            ...
            entry_fb::ColumnValues::BoolValues => match self.fb.logical_column_type() {
                ...
                entry_fb::LogicalColumnType::Field => InfluxColumnType::Field(InfluxFieldType::Boolean),
                _ => unreachable!(),
            },
