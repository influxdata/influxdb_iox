...
",
    ...
    assert_eq!(f.timestamps, [ns_since_epoch], "in frame 3");
    assert_eq!(f.values, [0.64], "in frame 3");

    let f = assert_unwrap!(&frames[4], Data::FloatPoints, "in frame 4");
    assert_eq!(f.timestamps, [ns_since_epoch + 4], "in frame 4");
    assert_eq!(f.values, [0.000_003], "in frame 4");
    ...
    let f = assert_unwrap!(&frames[6], Data::FloatPoints, "in frame 6");
    assert_eq!(f.timestamps, [ns_since_epoch + 1], "in frame 6");
    assert_eq!(f.values, [27.99], "in frame 6");
