...
#[macro_export]
/// If `TEST_INTEGRATION` is set and Kafka is available via `docker-compose`, set up the Kafka
/// service as requested and return it to the caller.
///
...
static DOCKER_COMPOSE_YML: OnceCell<PathBuf> = OnceCell::new();

impl KafkaServer {
    fn new() -> Self {
        let docker_compose_yml = DOCKER_COMPOSE_YML.get_or_init(|| {
            let this_file_location = file!();
            let mut this_file_location: PathBuf = this_file_location.into();
            this_file_location.pop();
            this_file_location.pop();
            this_file_location.pop();
            this_file_location.push("docker");
            this_file_location.push("ci-kafka-docker-compose.yml");
            this_file_location
        });
        ...
        Command::new("docker-compose")
            .arg("-f")
            .arg(docker_compose_yml)
            .arg("up")
            .arg("-d")
            .spawn()
            .expect("starting of docker kafka/zookeeper process");

        let server_process = Command::new("docker-compose")
            .arg("-f")
            .arg(docker_compose_yml)
            .arg("logs")
    ...
    fn drop(&mut self) {
        ...
        Command::new("docker-compose")
            ...
            .arg(
                DOCKER_COMPOSE_YML
                    .get()
                    .expect("Should have set Docker compose yml location in new"),
            )
            .arg("down")
            .stdout(Stdio::null())
            .status()
