...
use internal_types::schema::{InfluxColumnType, InfluxFieldType, TIME_DATA_TYPE, IOxValueType};
...
impl Column {
    pub fn new(row_count: usize, column_type: InfluxColumnType) -> Self {
        ...
        let data = match column_type {
            InfluxColumnType::IOx(IOxValueType::Boolean) | InfluxColumnType::Field(InfluxFieldType::Boolean) => {
                let mut data = BitSet::new();
                data.append_unset(row_count);
                ColumnData::Bool(data, StatValues::default())
            }
            InfluxColumnType::IOx(IOxValueType::U64) | InfluxColumnType::Field(InfluxFieldType::UInteger) => {
                ColumnData::U64(vec![0; row_count], StatValues::default())
            }
            InfluxColumnType::IOx(IOxValueType::F64) | InfluxColumnType::Field(InfluxFieldType::Float) => {
                ColumnData::F64(vec![0.0; row_count], StatValues::default())
            }
            InfluxColumnType::IOx(IOxValueType::I64) | InfluxColumnType::Field(InfluxFieldType::Integer) | InfluxColumnType::Timestamp => {
                ColumnData::I64(vec![0; row_count], StatValues::default())
            }
            InfluxColumnType::IOx(IOxValueType::String) | InfluxColumnType::Field(InfluxFieldType::String) => ColumnData::String(
    ...
    pub fn to_arrow(&self) -> Result<ArrayRef> {
        ...
        let data: ArrayRef = match &self.data {
            ...
            ColumnData::I64(data, _) => match self.influx_type {
                ...
                InfluxColumnType::IOx(IOxValueType::I64) | InfluxColumnType::Field(InfluxFieldType::Integer) => {
