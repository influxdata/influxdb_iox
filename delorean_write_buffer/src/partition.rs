...
/// Describes the result of translating a set of strings into the ids
/// specific to a partition
#[derive(Debug, PartialEq, Eq)]
pub enum PartitionIdSet {
    /// At but at least one of the srings was not present in the
    /// partitions' dictionary (so if we are testing for the presence
    /// of all ids we know it can not be true)
    AtLeastOneMissing,
...
pub struct PartitionPredicate {
    ...
    // and field columns returnedto only those named
    ...
    /// If Some, then the table must contain all all columns specified
...
impl PartitionPredicate {
    ...
    /// in plans which select a subset of fields, returns true if the
    /// field should be incuded in the results
